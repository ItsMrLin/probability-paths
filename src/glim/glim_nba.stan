functions {
  matrix[] calc_covars(matrix X, vector sigma2, vector beta, real rho, int T_) {
    int n_obs = rows(X);
    ...
    slopes = 5/(T_-1) * inv_logit(X * beta);

    for (i in 1:n_obs){
      vector[T_] vals = slopes[i]*diag_seq;
      sigma = sqrt(exp(vals));
...
parameters {
  ...
  // limit is around +-tanh(0.5)
  real atanh_rho;
  // real<lower=-0.55, upper=0.55> atanh_rho;
  vector[K] beta;
}
...
transformed parameters {
  ...
  sigma2[1] = 0;
  for (i in 2:T_) {
    // sigma2[i] = log(1 + exp(pre_tsfm_sigma2[i-1]));
    sigma2[i] = 0;
  }
}
...
model {
  ...
  covars = calc_covars(X, sigma2, beta, rho, T_);
